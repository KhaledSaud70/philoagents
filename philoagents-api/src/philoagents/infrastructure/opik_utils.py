...
def configure() -> None:
    if settings.COMET_API_KEY and settings.COMET_PROJECT:
        ...
        try:
            ...
            logger.warning(
                "Failed to configure Opik. Possilbe causes: missing/invalid COMET_API_KEY or COMET_PROJECT_NAME "
                "environment variables, or an issue connecting to the Opik server. Error: %s",
                e,
            )
    else:
        logger.warning(
            "COMET_API_KEY and COMET_PROJECT_NAME are not set. Set them to enable prompt monitoring with Opik (powered by Comet ML)."
        )
